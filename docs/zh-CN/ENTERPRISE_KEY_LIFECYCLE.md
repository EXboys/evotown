...
| 平台管理员 | `console.write` + admin | 控制台管理、上传 skill、发布 Bundle |
...
控制台支持 OIDC 授权码登录（环境变量 `EVOTOWN_OIDC_ISSUER`、`EVOTOWN_OIDC_CLIENT_ID`、`EVOTOWN_OIDC_CLIENT_SECRET`，以及 `EVOTOWN_PUBLIC_URL` 或 `EVOTOWN_OIDC_REDIRECT_URI`）。

- 登录页：**企业 SSO 登录** → `/api/v1/auth/oidc/start`
- 回调后一次性 `oidc_code` → `/api/v1/auth/oidc/exchange` 换取 `evk_` session key

生产环境建议配置 IdP 并计划后续 JWKS 验签加固（见路线图）。
...
- 自动 key 轮换 webhook
- OIDC id_token JWKS 强制验签（当前为 MVP payload 解析）

参见 [ENTERPRISE_QUICKSTART.md](./ENTERPRISE_QUICKSTART.md)、[MDM_AGENT_ROLLOUT.md](./MDM_AGENT_ROLLOUT.md)、[PRIVATE_SKILLS_MARKET_DEPLOYMENT.md](./PRIVATE_SKILLS_MARKET_DEPLOYMENT.md)
