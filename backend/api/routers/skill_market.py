...
async def get_skill_agent_versions(skill_id: str):
    ...
    import yaml
    from infra import agent_skills as agt_skills, agents as agents_store

    skill = skill_market.get_skill(skill_id)
    market_version = (skill.get("version") or "").strip() if skill else ""

    # Find agents that have this skill bound
    agent_ids = agt_skills.list_agents_for_skill(skill_id)
    results: list[dict] = []
    ...
    for aid in agent_ids:
        ...
        agent = agents_store.get_agent(aid)
        if agent is None:
            continue
